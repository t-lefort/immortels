...
- Role is shown behind a press-and-hold gesture and can be re-opened any time via "Ma carte". A screenshot cannot be prevented (no browser API can, and a second phone defeats anything client-side), so the card is instead made worthless as proof: **any player can display a Villageois card**, and the real/fake selector is hidden while the card is held, so a capture cannot reveal which mode produced it
- Only the villager card is fakeable, deliberately. A fake wolf card would need an invented pack, and an old screenshot freezes that pack while the village keeps learning roles — the day one of its names is publicly cleared, the screenshot proves its holder was never a wolf. Nobody needs to pretend to be a wolf, so the mode does not exist
