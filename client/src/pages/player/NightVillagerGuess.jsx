...
export default function NightVillagerGuess() {
  ...
  const alreadyGuessed = hasVoted.villager_guess;

  // Alive players excluding self
  const guessableTargets = players.filter(
  ...
  async function handleConfirm() {
    ...
    setError(null);
    try {
      await villagerGuess(selected.id);
      setConfirming(false);
      setSelected(null);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-6 pb-28">
      ...
      {!alreadyGuessed && selected && !confirming && (
